...
每个已注册流程的本地凭据检查只读取一次存储凭据。若存有 OAuth 凭据，则报告提供方是否支持 OAuth，不执行刷新；否则使用提供方公开的 `apiKey.check`，没有该检查时使用 `apiKey.resolve`，并传入已读取的凭据和常规认证上下文。单次读取可避免并发存入的 OAuth 凭据在检查期间触发刷新。哪些原生凭据有效由已安装的提供方决定，包括 Kimi 的 `KIMI_API_KEY` 和 GitHub Copilot 的 `COPILOT_GITHUB_TOKEN`；UI 派生的 `<ROUTE>_API_KEY` 名称不能证明提供方会读取它。检查可能在 Host 内解析机密值，但只返回布尔值。
...
- **凭据检查只确认本地存在性，不验证账号**——已存储的 OAuth 凭据不会被刷新，也不会向远程服务验证。检测到原生凭据不能证明账号有效、具有使用权限或请求能够成功。
