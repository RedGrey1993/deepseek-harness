...
Each registered flow's local credential check reads its stored credential once. A stored OAuth credential reports whether the provider supports OAuth, without refresh; otherwise the check uses the provider's public `apiKey.check`, or `apiKey.resolve` when no check exists, with that captured credential and the normal auth context. The single read prevents a concurrently stored OAuth credential from triggering refresh during the check. The installed provider determines which native credentials count, including `KIMI_API_KEY` for Kimi and `COPILOT_GITHUB_TOKEN` for GitHub Copilot; a UI-derived `<ROUTE>_API_KEY` name is not evidence that the provider reads it. Checks may resolve secret values inside the Host, but return only a boolean.
...
The installed pi-ai release supplies the built-in model directory. Signing in and fetching available models do not update that release. Dependency upgrades can add or remove model ids; saved `models` lists remain explicit selections, and removed ids are never silently redirected to another model. Clear the custom list to inherit the installed directory, or adopt discovered entries explicitly.
...
For self-hosted Chat Completions endpoints, `thinkingTokenBudgetField` selects the reasoning-budget parameter, and `vllmPriority` sets an integer scheduler priority when the server enables priority scheduling. Template arguments accept `$var: thinking.budget`. `openai-responses` gateways can set `supportsMaxOutputTokens: false` to omit `max_output_tokens`; Azure and Codex transports ignore this shared compatibility field. These controls are opt-in; transcript capabilities for mid-conversation system messages and tool changes, Anthropic effort and fallback capabilities, and provider session-affinity formats remain catalog-owned.
...
- **Credential checks are local presence checks, not account validation** — a stored OAuth credential is not refreshed or checked against the remote service. A detected native credential does not establish account validity, entitlement, or request success.
...
- **Only a leading in-history `system` message becomes pi-ai's `systemPrompt`** — this adapter folds a later `system` message, or a leading one when `GenerateOptions.system` is also set, into a `user` message at its position; provider-specific placement of the prompt follows pi-ai rather than a harness-owned wire override. Images in system or assistant history, including the leading system message, fail with `UNSUPPORTED_CONTENT` on both conversion paths.
