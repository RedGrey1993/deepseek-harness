...
Built-in pi-ai editors offer account controls when the installed provider advertises OAuth, including `xai` (SuperGrok or X Premium) and `openai-codex` (ChatGPT). The generic account entry follows installed catalog methods rather than a hardcoded provider list. The card displays the provider's login-method label and retains the API-key input when the provider also supports API keys, as xAI does, or the profile explicitly names `apiKeyEnv`; OAuth-only providers otherwise hide it. API-only and custom providers have no account controls. An explicit `apiKeyEnv` overrides account authentication and disables sign-in; remove that reference from the profile and clear any typed API-key draft to enable sign-in. Account controls and metadata requests are loopback-only. A failed account metadata refresh retains the last-confirmed methods and state and displays the failure without cancelling an active login. The card shows provider-issued authorization links, device codes, and prompts, and cancels its private login stream on close. An explicit `apiKeyEnv` requires its own confirmed credential, even when an account or native credential exists. Without that override, an active OAuth-capable route requires a stored account credential or Host-confirmed `nativeConfigured`; unknown or unavailable authorization alone never establishes readiness. A derived reference is only an editing and storage address, not native credential evidence. Sign-out removes the Harness account credential but keeps the provider profile; deleting the provider row keeps the account grant.
