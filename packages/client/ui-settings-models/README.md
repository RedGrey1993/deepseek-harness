...
The built-in `openai-codex` editor offers ChatGPT sign-in instead of an API-key field. The card shows provider-issued authorization links, device codes, and prompts, and cancels its private login stream on close. Account controls are offered on localhost, following the settings page's Host-account policy. Stored OAuth state determines whether a reference-free Codex route is ready. Sign-out removes its Harness credential; deleting its provider row keeps that credential.
