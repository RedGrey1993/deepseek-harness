...
当已安装的提供方声明支持 OAuth 时，内置 pi-ai 编辑器会提供账号控件，包括 `xai`（SuperGrok 或 X Premium）和 `openai-codex`（ChatGPT）。通用账号入口依据已安装目录中的方法，而非硬编码的提供方列表。卡片展示提供方的登录方式标签；若提供方也支持 API 密钥（如 xAI），或 profile 显式指定了 `apiKeyEnv`，则保留 API 密钥输入框，否则仅支持 OAuth 的提供方会隐藏该输入框。仅支持 API 密钥的提供方和自定义提供方没有账号控件。显式 `apiKeyEnv` 会覆盖账号认证并禁用登录；要启用登录，需从 profile 中移除该引用，并清空已键入的 API 密钥草稿。账号控件与元数据请求仅限回环访问。账号元数据刷新失败时，页面保留最近确认的登录方式与状态并展示错误，不会取消正在进行的登录。卡片展示提供方发出的授权链接、设备码和问题，并在关闭时取消自己的登录流。即使存在账号或原生凭据，显式 `apiKeyEnv` 仍要求其自身的凭据已确认配置。没有该覆盖时，活动的 OAuth 路由需要已存储的账号凭据或 Host 确认的 `nativeConfigured`；授权未知或不可用本身绝不意味着就绪。派生引用仅是编辑与存储地址，不是原生凭据的证据。退出登录移除 Harness 账号凭据，但保留提供方 profile；删除提供方行则保留账号授权。
