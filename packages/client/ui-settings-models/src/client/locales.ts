...
export const en = {
  ...
  oauthOverride: 'An API key overrides account sign-in. Clear the API key input and remove this route’s apiKeyEnv in settings.yaml to use account sign-in.',
...
export const zh: { [Key in keyof typeof en]: string } = {
  ...
  oauthOverride: 'API 密钥会覆盖账号登录。请清空 API 密钥输入框，并在 settings.yaml 中移除此路由的 apiKeyEnv 后使用账号登录。',
