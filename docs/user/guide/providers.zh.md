...
对于 `openai-codex`，通过 localhost 打开 Host，选择**使用 ChatGPT 登录**，再打开显示的授权链接并按提示操作。提供方需要验证码或输入时，卡片会显示对应内容。登录后选择**应用**保存提供方，再选择 Codex 模型。关闭卡片或选择**取消**会取消当前登录尝试。**退出登录**只移除 Harness 自己保存的账号凭据。如果路由已经指定 `apiKeyEnv`，请先在 `settings.yaml` 中移除该显式引用，再使用账号登录；否则它会覆盖已保存的 OAuth 凭据。
