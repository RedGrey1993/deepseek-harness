...
| `ctx.authorizationController` | `core` | [`api-settings-controller`](../packages/api/settings-controller) | - | - | - | Projects provider authorization onto caller-private streams and scoped prompt replies without exposing stored credentials. |
