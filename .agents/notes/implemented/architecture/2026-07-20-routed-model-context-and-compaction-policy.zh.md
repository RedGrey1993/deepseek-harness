...
顶层 `summaryInstruction` 只替换辅助摘要请求的最终 user 消息。它在按准确目标解析策略后仍保留，路由、取消、输出校验和保留范围继续由现有摘要实现负责。准确的自定义文本随 `compaction/summary` 记录；省略时保留内置指令。将此配置放在既有后端，可避免各领域预设通过子类重复实现摘要请求。
