...
A top-level `summaryInstruction` replaces only the final auxiliary user message. It survives exact-target policy resolution, while routing, cancellation, output validation and retention remain owned by the existing summarizer. The exact custom text is recorded with `compaction/summary`; omission retains the built-in directive. Keeping this configuration on the existing backend avoids requiring each domain preset to duplicate the summarizer through a subclass.
